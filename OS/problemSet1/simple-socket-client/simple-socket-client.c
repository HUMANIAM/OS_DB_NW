...
void configureConnection(int* socket_fd, struct sockaddr_in * server_socket_addr){
...
int main(int argc, char **argv) {
  ...
  char* msg = "are you ready server?";

   //configure connection with the server
   configureConnection(&socket_fd, &server_socket_addr);  

  // Connect socket to server
  if (0 > connect(socket_fd, (struct sockaddr *)&server_socket_addr, sizeof(struct sockaddr_in))) {
