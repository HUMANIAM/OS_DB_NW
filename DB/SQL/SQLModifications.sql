...
select * 
from Apply, Student
where Apply.sID = Student.sID and  major = 'EE' and GPA >= all (
			-- GPA of students applied to EE major
			select GPA 
			from Student
			where sID in (select GPA from AppliedStudentsEE)
			);

-- using views
create view AppliedStudentsEE as
