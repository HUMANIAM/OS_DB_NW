...
select count(*)
from Student
where Student.sID in (select distinct Student.sID from Student join Apply on Student.sID = Apply.sID where cName = 'Cornell');
...
select (CS.avgGPA - NonCS.avgGPA) as d
...
select cName, count(*) as AppsNumber
from Apply
group by cName;
...
select cName
from College
where (select count(*) from Apply where Apply.cName = College.cName) < 5;
...
/*** Back to original Group-by form, fewer than 5 distinct applicants ***/
